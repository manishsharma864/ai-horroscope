...
if user_input:
    ...
    if st.session_state.step == "name":
        ...
        response = f"Got it! Your birth details: {st.session_state.dob}, {st.session_state.tob}, {st.session_state.place}. Would you like a personal reading or to match with a partner?"
        st.session_state.step = "choice"
    
    elif st.session_state.step == "choice":
        if user_input.lower() in ["personal", "personal reading"]:
            prompt = f"Provide a detailed Vedic astrology reading for {st.session_state.name}, born on {st.session_state.dob} at {st.session_state.tob} in {st.session_state.place}."
            response = generate_text(prompt)
            st.session_state.step = "done"
        elif user_input.lower() in ["partner", "match", "partner matching"]:
            response = "Please enter your partner's name."
            st.session_state.step = "partner_name"
        else:
            response = "Please choose: 'personal reading' or 'partner matching'."
        ...
        prompt = f"Perform a Vedic astrology partner matching (Guna Milan) for {st.session_state.name} (born {st.session_state.dob} at {st.session_state.tob} in {st.session_state.place}) and {st.session_state.partner_name} (born {st.session_state.partner_dob} at {st.session_state.partner_tob} in {st.session_state.partner_place}). Provide a compatibility score out of 36 and detailed insights."
        response = generate_text(prompt)
        st.session_state.step = "done"
